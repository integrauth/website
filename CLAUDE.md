...
├── src/worker.ts + src/lib/server/{api,session,store,certs,security,env}.ts (Cloudflare Worker: static assets + /api/academy/*)
...
├── js/academy-auth.js (~36KB — site-wide "who am I" layer: navbar sign-in control, OTP login overlay, account panel, /api/academy/* client; loaded on EVERY page)
...
- **Ownership split**: the Lab owns the `users` and `sessions` tables *and* every session-mutating flow (email OTP signup/login, logout, revoke-all, TOTP step-up, account erase). This repo NEVER creates, refreshes or revokes a session — `src/lib/server/session.ts` only validates the shared cookie `__Secure-ia_session` (SHA-256 of the token vs `sessions.token_hash`, plus revoked/expiry/400-day-idle checks) and touches `last_seen_at`. Never run migrations from this repo against `lab-db`, and never write a Lab-owned table from here.
- **This repo's own tables**: `profiles` (first/last name + `name_locked_at`), `academy_lesson_progress`, `academy_quiz_progress`, `academy_last_position`, `academy_exam_attempts`, `academy_certificates` — all accessed through the parameterized helpers in `src/lib/server/store.ts`.
- **API** (`src/lib/server/api.ts`, Hono, base path `/api/academy`): `GET/PUT /profile`, `GET /progress`, `POST /progress/sync`, `POST|GET /exam/attempts`, `POST /certificates/issue`, `GET /certificates`, `GET /certificates/:serial/jwt`, plus two unauthenticated routes — `GET /certificates/verify/:serial` (public verification by serial) and `GET /.well-known/jwks.json` (public key for the certificate JWT). Everything else requires the session cookie. Merge semantics are deliberate: read-lessons union (`INSERT OR IGNORE`), quiz masks bitwise-OR per track, last-position last-write-wins by caller timestamp — a stale device can never delete progress. Known accepted gaps, documented in the code: no rate limiting (needs a DO/KV), and exam scores are client-asserted (no server-side answer key exists).
- **Client** (`js/academy-auth.js`, loaded on ALL 11 pages, exposes `window.AcademyAuth`): cross-origin credentialed calls to `https://lab.integrauth.com` for anything session-shaped (`/api/account`, `/api/signup/start`, `/api/signup/verify`, `/api/logout`, `/api/account/revoke-all`, `/api/session/revoke`, `/api/account/erase`) and same-origin calls to `/api/academy/*` for Academy data. Ships the navbar sign-in/account dropdown (`#acadAuthNav`, identical markup on every page), the OTP login overlay (email → 6-digit code, Cloudflare Turnstile widget, site key in the `turnstile-site-key` meta tag on every page), the themed confirm dialog, and academy.html's `#acadAccount` panel. Session state is cached per-tab in `sessionStorage` and broadcast via the `academy-auth-changed` DOM event; functions.js listens to it to kick off a progress sync.
- **Deleting an account deletes it in BOTH places** (`POST /api/account/erase` on the Lab) — progress, exam attempts, certificates and passkeys included. The UI requires typing DELETE. If the account has TOTP, the Lab returns `step_up_required` and the user is told to delete from lab.integrauth.com instead.
