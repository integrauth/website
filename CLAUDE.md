...
4. **Solutions** - Single auto-scrolling marquee row (`.services-marquee > .services-track > .service-slide`) with all 13 service cards; each card carries a `.service-tag` category pill ("AI & Agent Security" on the first 5: AI Integration Services, Agent Auth & Non-Human Identity, MCP Security & Secure Libraries, Fine-Grained Authorization, Securing AI Applications; "Identity, API & Engineering" on the last 8: Cloud-Native IAM, API Security Consulting, Enterprise SSO, Custom Software Development, Architecture & Design, Customer Identity & Advanced Authentication, Identity Threat Detection & Response, App Development (Android/iOS/Windows/Linux/web, added 2026-07-17)). Marquee pauses on hover/focus/touch, supports mouse drag + native touch scroll, loops seamlessly via cloned slides, and respects `prefers-reduced-motion`
