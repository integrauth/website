...
- **Tech Stack**: 19 collapsed categories (`tech-category-header` + `collapse`). CI/CD and DevSecOps are their own categories; "Infrastructure as Code" keeps only IaC/containers (delivery tooling moved out of the old "IaC & Automation"). Vendor sites are blocked in the sandbox — new brand logos come from the `simple-icons` npm package, coloured with the brand hex; near-black ones (Vercel, Helm, Dependabot, MCP) need the `.tech-logo[alt="…"]` white chip for dark themes and contrast hover.
