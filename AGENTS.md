...
- **Who** (`#who`): 4 `.process-step.who-card`s (security/AI product vendors, data/AI platforms, enterprises/integrators, teams who need a product built → `/products`); `.who-grid` is explicit 1/2/4 columns, not auto-fit (four cards orphan at 1200–1400px).
...
- **Tech Stack**: 19 collapsed categories (`tech-category-header` + `collapse`). CI/CD and DevSecOps are their own categories; "Infrastructure as Code" keeps only IaC/containers (delivery tooling moved out of the old "IaC & Automation"). Every card is a link — concept cards (Policy as Code, mTLS, SBOM…) point at the spec/OWASP page. Vendor sites are blocked in the sandbox, only `raw.githubusercontent.com` resolves: brand logos come from the `simple-icons` npm package (coloured with the brand hex) or a vendor's own repo; near-black ones (Vercel, Helm, Dependabot, GitHub Actions, Gitleaks, MCP) need the `.tech-logo[alt="…"]` white chip for dark themes and contrast hover.
